...
$stmt_public_user = $mysqli->prepare($sql_public_user);
$stmt_public_user->bind_param('i', $profile_user_id);
$stmt_public_user->execute();
$result_public_user = $stmt_public_user->get_result();
$public_user = $result_public_user->fetch_assoc();
...
$sql_common_orgs = "SELECT o.id AS org_id, o.name AS org_name, o.description AS org_description
                    ...
                    ORDER BY o.name ASC";
$stmt_common_orgs = $mysqli->prepare($sql_common_orgs);
$stmt_common_orgs->bind_param('ii', $logged_in_user_id, $profile_user_id);
$stmt_common_orgs->execute();
$result_common_orgs = $stmt_common_orgs->get_result();
$common_organizations_data = $result_common_orgs->fetch_all(MYSQLI_ASSOC);
$stmt_common_orgs->close();
...
if (!empty($common_organizations_data)) {
    ...
    if (!empty($placeholders)) {
        ...
        $types = 'i' . str_repeat('i', count($common_org_ids));
        $params_to_bind = array_merge([$profile_user_id], $common_org_ids);

        $bind_params_ref = [];
        foreach ($params_to_bind as $key => $value) {
            $bind_params_ref[$key] = &$params_to_bind[$key];
        }
        array_unshift($bind_params_ref, $types);

        call_user_func_array([$stmt_hours, 'bind_param'], $bind_params_ref);
