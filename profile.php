...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_profile') {
    ...
    if (empty($name) || empty($prenume)) {
        ...
        if ($result_check_email->num_rows > 0) {
            ...
            $sql_update = 'UPDATE users SET name = ?, prenume = ?, email = ?, updated_at = NOW() WHERE id = ?';
            $stmt_update = $mysqli->prepare($sql_update);
            $stmt_update->bind_param('sssi', $name, $prenume, $email, $user_id);
...
$stmt_organizations = $mysqli->prepare($sql_organizations);
$stmt_organizations->bind_param('i', $user_id);
$stmt_organizations->execute();
$result_organizations = $stmt_organizations->get_result();
$organizations = $result_organizations->fetch_all(MYSQLI_ASSOC);
$stmt_organizations->close();
